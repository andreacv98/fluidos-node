...
// NodeRecociler reconciles the detected cluster resources
...
func (r *NodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !labelSelector.Matches(labels.Set(node.GetLabels())) {
		//klog.Info("Node is not a resource node for FLUIDOS")
		return ctrl.Result{}, nil
	}
	...
	nodeInfo, err := GetNodeInfos(ctx, r.Client, &node, nodeMetrics)
	if err != nil {
	...
	for _, flavor := range flavorsList.Items {
...
func (r *NodeReconciler) createFlavor(ctx context.Context, nodeInfo *models.NodeInfo, nodeIdentity nodecorev1alpha1.NodeIdentity, ownerReferences []metav1.OwnerReference) (flavor *nodecorev1alpha1.Flavor, err error) {
